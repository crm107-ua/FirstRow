<%@ Page Title="" Language="C#" MasterPageFile="~/White.Master" AutoEventWireup="true" CodeBehind="Propuestas.aspx.cs" Inherits="FirstRow.Pages.Propuestas" %>
<asp:Content ID="Content1" ContentPlaceHolderID="Main" runat="server">
     <div class="image_header" runat="server" id="background_image_header" style="z-index: -3;"></div> 
    <div class="breadcrumbs">
            <div class="wrap">
                <div class="wrap_float">
                    <a href="/" style="color:white;">FirstRow</a>
                    <span class="separator" style="color:white;">/</span>
                    <a href="/propuestas" style="color:white;">Propuestas</a>
                     <a id="add_form" runat="server" style="float: right; color:white;" href="/crear_propuesta">Agregar propuesta</a>
                </div>
            </div>
        </div>
        <div class="page blog-list-page full-width">
            <div class="wrap">
                <div class="wrap_float">
                    <div class="page_head">
                <!-- <h1 class="title" runat="server" style="color:white;" id="sorteos_title"></h1>-->
                       <h1 class="title"><a style="color:white" href="/"><asp:Label ID="propuestas_title2" runat="server"></asp:Label></a></h1>
                        <br />
                        <p class="subtitle" runat="server" style="color:white;" id="propuestas_subtitle"></p>
                       
                    </div>
                    <div class="page_body">
                        <div class="blog-list" runat="server" id="sorteos_list">
                            
                            <!--
                           
                            <a class="blog_item" href="sorteo/sorteo-de-experiencia-por-el-atlantico">
                                <div class="blog_item_top" style="background-image: url(https://p4.wallpaperbetter.com/wallpaper/34/750/436/sunset-brittany-le-conquet-finistere-wallpaper-preview.jpg);">
                                    <div class="sq_parent">
                                        <div class="sq_wrap">
                                            <div class="sq_content">
                                                <div class="tags">
                                                    <div class="tag red">
                                                        Travel S.A
                                                    </div>
                                                </div>
                                                <h3 class="_title">
                                                    El Atlántico, un oceano inolvidable
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="shadow js-shadow"></div>
                                </div>
                                <div class="blog_item_bottom">
                                    <div class="author">
                                        <div class="userpic">
                                            <img alt="" src="https://t3.ftcdn.net/jpg/03/46/83/96/360_F_346839683_6nAPzbhpSkIpb8pmAwufkC7c5eD7wYws.jpg">
                                        </div>
                                        <p class="date">
                                            21.09.2022 - 1760 participantes
                                        </p>
                                    </div>
                                </div>
                            </a>
                            <a class="blog_item" href="sorteo/sorteo-de-experiencia-por-el-mediterraneo">
                                <div class="blog_item_top" style="background-image: url(https://www.10wallpaper.com/wallpaper/1920x1080/1401/alicante_night-HD_photography_wallpaper_1920x1080.jpg);">
                                    <div class="sq_parent">
                                        <div class="sq_wrap">
                                            <div class="sq_content">
                                                <div class="tags">
                                                    <div class="tag black">
                                                        Jet Dreams
                                                    </div>
                                                </div>
                                                <h3 class="_title">
                                                    Lo mejor del Mediterráneo
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="shadow js-shadow"></div>
                                </div>
                                <div class="blog_item_bottom">
                                    <div class="author">
                                        <div class="userpic">
                                            <img alt="" src="https://t3.ftcdn.net/jpg/03/46/83/96/360_F_346839683_6nAPzbhpSkIpb8pmAwufkC7c5eD7wYws.jpg">
                                        </div>
                                        <p class="date">
                                            21.09.2022 - 1000 participantes
                                        </p>
                                    </div>
                                </div>
                            </a>
                            <a class="blog_item" href="sorteo/sorteo-de-experiencia-por-italia">
                                <div class="blog_item_top" style="background-image: url(https://www.expreso.info/files/2021-06/Italia_Roma.jpg);">
                                    <div class="sq_parent">
                                        <div class="sq_wrap">
                                            <div class="sq_content">
                                                <div class="tags">
                                                    <div class="tag blue">
                                                        LeFrance
                                                    </div>
                                                </div>
                                                <h3 class="_title">
                                                    Italia, vestigios de una gran civilizacion
                                                </h3>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="shadow js-shadow" style="background-image: url(https://www.expreso.info/files/2021-06/Italia_Roma.jpg);"></div>
                                </div>
                                <div class="blog_item_bottom">
                                    <div class="author">
                                        <div class="userpic">
                                            <img alt="" src="https://t3.ftcdn.net/jpg/03/46/83/96/360_F_346839683_6nAPzbhpSkIpb8pmAwufkC7c5eD7wYws.jpg">
                                        </div>
                                        <p class="date">
                                            21.09.2019 - 1240 participantes
                                        </p>
                                    </div>
                                </div>
                            </a>  
                            aqui
                            -->
                        </div>
                        
                        
                        
                    </div>
                </div>
            </div>
        </div>
</asp:Content>